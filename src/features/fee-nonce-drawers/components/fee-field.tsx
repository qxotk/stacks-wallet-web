...
export const FeeField = ({ byteSize, ...props }: FeeField) => {
  ...
  const [customAbsoluteFee] = useCustomAbsoluteFee();

  console.log({ customAbsoluteFee, feeRate });
  ...
  const fieldError = useMemo(() => {
    switch (meta.error) {
      case SendFormErrorMessages.AdjustedFeeSurpassesBalance:
        return (
          <>
            The fee added now exceeds your current STX balance. Consider lowering the amount being
            sent.{' '}
            <Button variant="link" color="inherit" onClick={() => console.log('slkdfsdfa')}>
              Set max to: 100 STX
            </Button>
          </>
        );
